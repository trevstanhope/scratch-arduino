...
int HMC6352StartCalibrationAddress = 0x43; //"C" in hex, C command is: "StartCalibrate" command
int HMC6352EndCalibrationAddress = 0x45; //"E" in hex, E command is: "EndCalibrate" command
...
void HMC6352Class::StartCalibration()
{
  Wire.beginTransmission(HMC6352SlaveAddress);
  Wire.write(HMC6352StartCalibrationAddress); //C enter sleep mode
  Wire.endTransmission();
}
...
{
  Wire.beginTransmission(HMC6352SlaveAddress);
  Wire.write(HMC6352EndCalibrationAddress); //E enter sleep mode
  Wire.endTransmission();
}
